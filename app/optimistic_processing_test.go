...
// Test round change when old optimistic processing has completed - should restart
func (suite *OptimisticProcessingTestSuite) TestHandleRoundChange_CompletedRestarts() {
	require := suite.Require()

	// Set up completed round 0 processing
	oldCompletion := make(chan struct{}, 1)
	oldCompletion <- struct{}{} // Signal completion (round 0 finished)

	suite.app.optimisticProcessingInfoMutex.Lock()
	suite.app.optimisticGeneration = 1
	suite.app.optimisticProcessingInfo = OptimisticProcessingInfo{
		Height:     suite.ctx.BlockHeight(),
		Hash:       []byte("round0-hash"),
		Completion: oldCompletion,
		Generation: 1,
	}
	suite.app.optimisticProcessingInfoMutex.Unlock()

	// Call ProcessProposalHandler with a different hash (round change)
	req := &abci.RequestProcessProposal{
		Height: suite.ctx.BlockHeight(),
		Hash:   []byte("round1-hash"),
		Txs:    [][]byte{},
	}
	...
	// Verify new optimistic processing was started
	info := suite.app.GetOptimisticProcessingInfo()
	require.Equal([]byte("round1-hash"), info.Hash)
	require.NotNil(info.Completion)
	require.False(info.Aborted)
	require.Equal(uint64(2), info.Generation) // Generation should have incremented
}

// Test round change when old optimistic processing is still running - should abort
func (suite *OptimisticProcessingTestSuite) TestHandleRoundChange_StillRunningAborts() {
	require := suite.Require()

	// Set up in-progress round 0 processing (completion NOT signaled)
	oldCompletion := make(chan struct{}, 1)
	...
	// Verify processing was aborted (old goroutine still running)
	info := suite.app.GetOptimisticProcessingInfo()
	require.True(info.Aborted)
	// Original completion channel should be preserved
	require.Equal(oldCompletion, info.Completion)
}

// Test generation guard prevents stale goroutine from overwriting newer results
func (suite *OptimisticProcessingTestSuite) TestGenerationGuard_StaleGoroutine() {
	require := suite.Require()

	// Simulate: generation 1 goroutine finishes, but generation has already moved to 2
	gen2Completion := make(chan struct{}, 1)
	suite.app.optimisticProcessingInfoMutex.Lock()
	suite.app.optimisticGeneration = 2
	suite.app.optimisticProcessingInfo = OptimisticProcessingInfo{
		Height:     suite.ctx.BlockHeight(),
		Hash:       []byte("gen2-hash"),
		Completion: gen2Completion,
		Generation: 2,
	}
	suite.app.optimisticProcessingInfoMutex.Unlock()

	// Simulate a stale goroutine (generation 1) trying to write results
	done := make(chan struct{})
	go func() {
		defer close(done)
		myGeneration := uint64(1) // stale generation

		suite.app.optimisticProcessingInfoMutex.Lock()
		if suite.app.optimisticProcessingInfo.Generation != myGeneration {
			// Should hit this path - generation mismatch
			suite.app.optimisticProcessingInfoMutex.Unlock()
			return
		}
		// Should NOT reach here
		suite.app.optimisticProcessingInfo.Events = []abci.Event{{Type: "stale-event"}}
		suite.app.optimisticProcessingInfoMutex.Unlock()
	}()
	...
	case <-done:
	case <-time.After(time.Second):
		suite.T().Fatal("Timeout waiting for stale goroutine")
	}

	// Verify gen2 info was not overwritten
	info := suite.app.GetOptimisticProcessingInfo()
	require.Equal([]byte("gen2-hash"), info.Hash)
	require.Equal(uint64(2), info.Generation)
	require.Nil(info.Events) // Should NOT have stale events
}

// Test multiple round changes (0 -> 1 -> 2)
func (suite *OptimisticProcessingTestSuite) TestMultipleRoundChanges() {
	require := suite.Require()

	// Round 0: Start optimistic processing
	suite.app.ClearOptimisticProcessingInfo()
	suite.app.optimisticProcessingInfoMutex.Lock()
	suite.app.optimisticGeneration = 0
	suite.app.optimisticProcessingInfoMutex.Unlock()

	req0 := &abci.RequestProcessProposal{
		Height: suite.ctx.BlockHeight(),
		Hash:   []byte("round0-hash"),
		Txs:    [][]byte{},
	}
	resp, err := suite.app.ProcessProposalHandler(suite.ctx, req0)
	require.NoError(err)
	require.Equal(abci.ResponseProcessProposal_ACCEPT, resp.Status)

	info := suite.app.GetOptimisticProcessingInfo()
	require.Equal(uint64(1), info.Generation)

	// Simulate round 0 completing
	suite.app.optimisticProcessingInfoMutex.Lock()
	suite.app.optimisticProcessingInfo.Completion <- struct{}{}
	suite.app.optimisticProcessingInfoMutex.Unlock()

	// Give the channel a moment to be readable
	time.Sleep(10 * time.Millisecond)

	// Round 1: Different hash, old completed -> should restart
	req1 := &abci.RequestProcessProposal{
		Height: suite.ctx.BlockHeight(),
		Hash:   []byte("round1-hash"),
		Txs:    [][]byte{},
	}
	resp, err = suite.app.ProcessProposalHandler(suite.ctx, req1)
	require.NoError(err)
	require.Equal(abci.ResponseProcessProposal_ACCEPT, resp.Status)
	...
	// Round 2: Different hash again, old completed -> should restart again
	req2 := &abci.RequestProcessProposal{
		Height: suite.ctx.BlockHeight(),
		Hash:   []byte("round2-hash"),
		Txs:    [][]byte{},
	}
	resp, err = suite.app.ProcessProposalHandler(suite.ctx, req2)
	require.NoError(err)
	require.Equal(abci.ResponseProcessProposal_ACCEPT, resp.Status)

	info = suite.app.GetOptimisticProcessingInfo()
	require.Equal([]byte("round2-hash"), info.Hash)
	require.Equal(uint64(3), info.Generation)
	require.False(info.Aborted)
}
